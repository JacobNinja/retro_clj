...
(defn- t-own [offset key-length]
  (map (fn [i]
         (nth key-window
              (Math/abs (mod (+ offset i) (count key-window)))))
       (range key-length)))

(defn- t-given [key key-length]
  (reductions (fn [acc i]
                (if (= i 31)
                  key
                  (int (/ acc 2))))
...
(defn lookup-table [key key-length offset]
  (let [t (map (fn [[t-given t-own]]
                 (Math/abs (bit-xor t-given t-own)))
               (partition 2 (interleave (t-given key key-length)
                                        (t-own offset key-length))))]
    (take 256 (cycle t))))

(defn mix [{:keys [table i j] :as env}]
  (let [next-i (mod (inc i) 256)
        next-j (mod (+ j (nth table next-i)) 256)
        next-table (replace (merge {(nth table next-i) (nth table next-j)}
                                   {(nth table next-j) (nth table next-i)})
                            table)]
    {:table next-table
     :i next-i
     :j next-j
     :mix (next-table (mod (+ (nth next-table next-i)
                              (nth next-table next-j))
                           256))}))
...
(defn initialize-table [key]
  (let [key-length (/ (bit-and key 0xf8) 8)
        length (cond-> key-length
                       (< key-length 20) (+ 20))
        offset (mod key (count key-window))]
    (loop [u 0
           key-table (lookup-table key length offset)
           table (range 256)
           index 0]
      (if (empty? key-table)
        table
        (let [next-u (mod (+ u
          ...
          (recur next-u
                 (rest key-table)
                 (replace (merge {(nth table index) (nth table next-u)}
                                 {(nth table next-u) (nth table index)}) table)
                 (inc index)))))))

(defn- do-cipher [f coll env]
  (let [mixes (iterate mix (mix env))]
    (merge (last (take (count coll) mixes))
           {:result
            (apply str
                   (map f
                        (partition 2 (interleave coll mixes))))})))

(defn decipher [s env]
  (do-cipher (fn [[hex env]]
               (char (bit-xor (Integer/parseInt (apply str hex) 16)
                              (env :mix))))
             (partition 2 s)
             env))
...
(defn encipher [s env]
  (do-cipher (fn [[c env]]
               (let [b (bit-xor (int c) (env :mix))]
                 (if (<= b 0)
                   "00"
                   (apply str (map #(nth hex %)
                                   [(bit-and (bit-shift-right b 4) 15)
                                    (bit-and b 15)])))))
             s
             env))

(defn make-cipher [f table]
  (let [table (atom table)
        j (atom 0)
        i (atom 0)]
    (fn [s]
      (let [cipher-result (f s {:table @table
                              :j @j
                              :i @i})]
        (reset! table (cipher-result :table))
        (reset! i (cipher-result :i))
        (reset! j (cipher-result :j))
